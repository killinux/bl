...
| `unused*` | 13 | 1 根 rename 下半身，8 根 rename twist，4 根按顶点拆分到下半身/足D |
...
### 按顶点位置拆分 (4 根) — commit 9687975

全部通过 SPLIT_BONES 按顶点位置拆分，胯部以上→下半身，以下→足.L/足.R，再由 5.2 转移到足D。

| XPS 名称 | parent | 顶点 | 拆分结果 | 处理阶段 |
|----------|--------|------|----------|----------|
| `unused bip001 xtra04` | leg left thigh | 853 | 足.L(430v) + 下半身(423v) | Step 5.1 SPLIT + 5.2 |
| `unused bip001 xtra02` | leg right thigh | 850 | 足.R(429v) + 下半身(421v) | Step 5.1 SPLIT + 5.2 |
| `unused bip001 xtra08` | pelvis (下半身) | 1082 | 下半身(962v) + 足.L(120v) | Step 5.1 SPLIT + 5.2 |
| `unused bip001 xtra08opp` | pelvis (下半身) | 1228 | 下半身(1109v) + 足.R(119v) | Step 5.1 SPLIT + 5.2 |

历史：最初被 PRESERVE 保留为 hidden deform 骨 → commit 562076c 改为整体合并到足D（导致胯部变形）→ commit 9687975 改为按顶点拆分（修复变形）。
...
- **2026-04-12 commit 9687975**: xtra02/04/08/08opp 全部改为 SPLIT_BONES 按顶点拆分（修复整体合并导致的胯部变形）
- **2026-04-12 commit 562076c**: xtra02/xtra04 从 PRESERVE 移除（整体合并，后发现胯部变形，被 9687975 替代）
- **2026-04-12 commit 9dfd629**: 下半身 layer 修复（从 layer 1 拉回 layer 0）
- **2026-04-12 初版文档**: 建立全部 109 根骨的去向追踪

## 当前状态：全部 13 根 unused 骨已处理完毕（0 根未处理）
