...
| `unused*` | 13 | 1 根 rename 下半身，8 根 rename twist，2 根合并足D，2 根保留 hidden |
...
| XPS 名称 | parent | 顶点 | 转换后 | 处理阶段 |
|----------|--------|------|--------|----------|
| `unused bip001 xtra04` | leg left thigh | 853 | 权重 → 足.L → **足D.L** | Step 5.1 + 5.2 |
| `unused bip001 xtra02` | leg right thigh | 850 | 权重 → 足.R → **足D.R** | Step 5.1 + 5.2 |

之前被 PRESERVE 保留，导致足D权重占比只有 2.7%（目标 15.1%）。移除 PRESERVE 后权重正常合并。

### 未处理 (2 根) — 保留 hidden

| XPS 名称 | parent | 顶点 | 权重sum | 当前状态 |
|----------|--------|------|---------|----------|
| `unused bip001 xtra08` | pelvis (下半身) | 1082 | 123 | hidden, deform |
| `unused bip001 xtra08opp` | pelvis (下半身) | 1228 | 126 | hidden, deform |

臀部/大腿外侧辅助骨，跨两个区域。已有 SPLIT_BONES 按顶点拆分逻辑但被 PRESERVE 挡住。可能方案：从 PRESERVE 移除让 SPLIT 生效，按顶点位置分到 下半身 / 足。
